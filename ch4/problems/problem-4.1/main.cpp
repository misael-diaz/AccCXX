 ...
 *
 * Synopsis:
 * Solves problem 4-1, code compiles with gcc 11.1.0 and produces the
...
int main() {
	...
	if ( roster.is_open() ) {
		...
		while ( read(roster, record) ) {
			maxlen = std::max(record.name.size(), maxlen) ;
	...
	for (const auto& student: students) {
		try {
			double final = grade(student) ;
			report(maxlen, student.name, final) ;
